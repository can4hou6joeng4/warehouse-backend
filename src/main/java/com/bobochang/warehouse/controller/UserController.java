...
public class UserController {
    ...
    public Result authList(@RequestHeader(WarehouseConstants.HEADER_TOKEN_NAME) String clientToken) {
        ...
        //根据用户id查询用户权限(菜单)树
        List<AuthInfo> authTreeList = authInfoService.findAuthTree(currentUser.getUserId());
        //响应
        return Result.ok(authTreeList);
    }

    @GetMapping("/user-list")
    public Result userListPage(Page page, UserInfo user) {
        //执行业务
        page = userInfoService.queryUserPage(page, user);
        //响应
